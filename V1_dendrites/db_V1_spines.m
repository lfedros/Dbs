...
db(i).neuron_id      = 12;           % neuron ID within the brain
db(i).data_repo      = data_repo;   % where the data live
db(i).morph.expRef   = {}; % expref of morphological zstack {'animal_name', 'expDate', expnum} 
db(i).morph.tilt     = [0, 0];         % angle betwen brain surface and coverslip [rostro-caudal, medio-lateral]
db(i).morph.zoom     = ;         % imaging zoom for morphological zstack
db(i).morph.zStep    = ;           % spacing between planes of zstack in microns
db(i).morph.somaZ    = ;         % depth of the soma measured from pia in microns
% db(i).morph.apicalNode    = [];         % root node of the apical dendrite
db(i).morph.upSamp    = ;          % upsampling factor for morphological tracing
% db(i).morph.dendrotomy    = {0, []}; 
...
db(i).retino.somaIJ  = [53 170];   % position of the soma on the retinotopy FOV in pixels from top left corner
db(i).prefDir        = [];         % preferred orientation measured at the soma
